...
| DDR-023 | `apps/api/src/modules/movies/dto/movie.dto.ts` (`MovieResponseDto.isComingSoon`, `MovieListQueryDto.isComingSoon`), `movies.service.ts` (`toResponse` computes it, `findAllMovies` filters on it) — no migration, no stored column; `apps/mobile/src/features/booking/{services,effect,hooks}` now read/filter by it instead of deriving client-side                                                                                                                                                    |
| ADR-017 | `apps/api/src/modules/wallets/stripe.service.ts` (the only file that calls Stripe; Customer created lazily with an idempotency key; PaymentIntent created and confirmed server-side, idempotency key from the ledger row id), `wallets.service.ts` (`topUp`: pending row committed, then Stripe, then a second short transaction), `stripe-webhook.controller.ts` (signature-verified, `@SkipThrottle`, no JWT), `rawBody: true` in `main.ts`, `STRIPE_*` in `env.validation.ts`/`.env.example`         |
| DDR-024 | `apps/api/src/modules/wallets/entities/` + `1790240623162-AddWallets.ts` (three tables, `CHECK (balance >= 0)`, unique `stripe_payment_intent_id`, backfill `INSERT INTO wallets … SELECT id FROM users`); `wallets.service.ts` `settleSucceeded`/`settleFailed` (lock ledger row, guard `pending`, lock wallet, credit — the only writer of `balance`); `AuthService.register()` and `SeedService.seedAdmin` create the wallet in the user's transaction; `token-packages.data.ts` seeds the catalogue |
...
| `CLAUDE.md`, `apps/api/.env.example` | `pnpm db:up` starts the database                                                   | No `db:up` script exists — only `docker:up`, which runs `docker compose up -d` (builds and starts `postgres` **and** the production `app` container, not just the database)                           | root `package.json` scripts, `docker-compose.yml`                           |
...
| Record  | Waiting on                                                                                                                                                                                                                           |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| DDR-024 | The mobile half only. `apps/mobile`'s `features/wallet/services/wallet.ts` is still the stub that throws on every call, and its `WalletTransactionStatusSchema` still says `completed` where the API (and DDR-024) says `succeeded`. |

## Notes

- **Payments go slightly beyond ADR-017 in two places (24 Sep 2026).** Neither contradicts it.
  (1) `GET /wallet/top-ups/:id` reconciles a `pending` row against its PaymentIntent on read,
  through the same idempotent `settleSucceeded`/`settleFailed` the webhook uses, so a finished
  3-D Secure challenge is credited without `stripe listen` running locally. (2) The webhook
  settles only a row whose recorded PaymentIntent id, amount and currency all match the event,
  finding it by the `walletTransactionId` the API puts in the PaymentIntent's metadata — so it
  can settle a row before the synchronous path has written the PaymentIntent id. The webhook is
  left out of the OpenAPI document, so it is not in `@movea/api-contract`.
